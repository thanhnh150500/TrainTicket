...
<html lang="vi">
    ...
    <form method="post" action="${pageContext.request.contextPath}/manager/seats" accept-charset="UTF-8">
            ...
            <div class="col-md-6">
                ...
                <select name="carriage_id" class="form-select" required>
                    ...
                    <c:forEach items="${carriages}" var="c">
                        <option value="${c.carriageId}" ${s.carriageId != null && s.carriageId == c.carriageId ? 'selected' : ''}>
                            ${c.trainCode} - ${c.code} <c:if test="${not empty c.trainName}">(${c.trainName})</c:if>
                        </option>
                    </c:forEach>
                </select>
            </div>
        ...
        <div class="mt-3">
            ...
            <input type="text" name="code" class="form-control" value="${s.code}" required maxlength="20" placeholder="VD: 01A"/>
            <div class="form-text">Code phải là duy nhất trong **cùng** một toa.</div>
        </div>

        <div class="mt-3">
            <label class="form-label">Vị trí (tùy chọn)</label>
            <input type="text" name="position_info" class="form-control" value="${s.positionInfo}" maxlength="50" placeholder="VD: Tầng 1 / Dãy B"/>
        </div>
