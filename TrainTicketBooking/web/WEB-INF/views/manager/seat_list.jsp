...
<html lang="vi">
    ...
    <body class="p-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h3>Danh sách ghế</h3>
            <a class="btn btn-primary" href="${pageContext.request.contextPath}/manager/seats?op=new">
                <i class="bi bi-plus"></i> Thêm ghế
            </a>
        </div>
        ...
        <table class="table table-striped align-middle">
            <thead>
                <tr>
                    ...
                    <th style="width:160px;">Tàu</th>
                    <th style="width:120px;">Toa</th>
                    <th style="width:120px;">Mã ghế</th>
                    <th style="width:200px;">Hạng ghế</th>
                    <th>Vị trí</th>
            ...
            <tbody>
                <c:forEach items="${list}" var="x">
                    <tr>
                        <td><c:out value="${x.seatId}"/></td>
                        <td><c:out value="${x.trainCode}"/> <small class="text-muted">(<c:out value="${x.trainName}"/>)</small></td>
                        <td><c:out value="${x.carriageCode}"/></td>
                        <td><c:out value="${x.code}"/></td>
                        <td><c:out value="${x.seatClassName}"/> <small class="text-muted">(<c:out value="${x.seatClassCode}"/>)</small></td>
                        <td><c:out value="${x.positionInfo}"/></td>

                        <td class="d-flex gap-2">
                            <a class="btn btn-sm btn-outline-secondary"
                               href="${pageContext.request.contextPath}/manager/seats?op=edit&id=${x.seatId}">Sửa</a>

                            <form method="post" action="${pageContext.request.contextPath}/manager/seats" class="d-inline">
                                <input type="hidden" name="op" value="delete"/>
                                <input type="hidden" name="id" value="${x.seatId}"/>
                                <input type="hidden" name="_csrf" value="${sessionScope.csrfToken}"/>
                                <button type="submit" class="btn btn-sm btn-outline-danger"
                                        onclick="return confirm('Xóa ghế này?');">Xóa</button>
                            </form>
                        </td>
                    </tr>
                </c:forEach>
                <c:if test="${empty list}">
                    <tr><td colspan="7" class="text-center text-muted">Chưa có ghế nào.</td></tr>
                </c:if>
            </tbody>
        </table>
