...
<%-- (Gi giữ style của Manager: bg-white, navbar-light) --%>

<nav class="navbar navbar-expand-lg navbar-light bg-white sticky-top tt-nav border-bottom">
<div class="container">

    <!-- SỬA: Brand link về trang POS của Staff -->
    <a class="navbar-brand d-flex align-items-center fw-semibold" href="${ctx}/staff/pos">
        <i class="bi bi-train-front-fill me-2"></i> TrainTicket <span class="ms-1 text-muted">Staff</span>
    </a>
    ...
    <div class="collapse navbar-collapse" id="ttMgrNavbar">
        ...
        <!-- Left (SỬA: Chỉ giữ link của Staff) -->
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">

            <!-- Link Bán Hàng -->
            <li class="nav-item">
                <a class="nav-link ${ fn:contains(uri,'/staff/pos') ? 'active' : '' }"
                   href="${ctx}/staff/pos">Bán Hàng</a>
            </li>

            <!-- Link Quản lý Đơn (của Staff) -->
            <li class="nav-item">
                <a class="nav-link ${ fn:contains(uri,'/staff/orders') ? 'active' : '' }"
                   href="${ctx}/staff/orders">Quản lý Đơn</a>
            </li>
        </ul>
